...
export async function GET(
  ...
  try {
    ...
    const xmlBase64 = await bkavInstance.getInvoiceFileXMLBase64(
      einvoice.documentId,
    );

    if (xmlBase64?.Status !== 0) {
      throw new Error("Lỗi khi truy xuất file XML từ đối tác hóa đơn điện tử", {
        cause: 500,
      });
    }

    if (!xmlBase64?.Object?.XML) {
      throw new Error("Không tìm thấy file XML của hóa đơn điện tử", {
        cause: 404,
      });
    }
    ...
    return NextResponse.json({
      data: {
        base64: xmlBase64?.Object?.XML,
        name: `[${einvoice?.invoice_form}]_[${einvoice?.invoice_serial}]_[${einvoice?.e_invoice_number}].xml`,
      },
    });
  } catch (error: any) {
    return NextResponse.json(
